...
public class Calendar {
    ...
    public static void CreateBooking(Date DesiredDate){
        ...
        int RoomNumber = IntInput();
        
        
        //gets slot number
        int SlotNumber = BookingSlots(RoomNumber);
        ...
        if(ValidBooking(Username,RoomNumber,SlotNumber,DesiredDate) == false){
        ...
        //registers booking
        Booking myBooking = new Booking(Name,Username,RoomNumber,SlotNumber,DesiredDate);
        Bookinglist.add(myBooking);
        fileHandling.writeFileBookings(Bookinglist);
        
    }
    
    public static int BookingSlots(int RoomNumber){
        //Array of slot times
        String[] time = {"Error","Error","Error","Error"};

        //outputs slots based on room number
        switch(RoomNumber){
            case 1:
                time[0] = "8-8.30";
                time[1] = "9-11.30";
        ...
        System.out.println("Slot 1: "+time[0]+"\nSlot 2: "+time[1]+"\nSlot 3: "+time[2]+"\nSlot 4: "+time[3]);
        return IntInput();
    }
    ...
    public static Date Date(){
        ...
        int month = IntInput();
        int day = IntInput();
        //add validation chacks to date
        
        Date bookingDate = new Date(year-1900, month-1, day,0,0,0);   
        return bookingDate;
    }
    
    //displays info on the rooms
    ...
    public static void Menu(){
        //gets users desired date
        Date DesiredDate = Date();
        //setups variables
        SetupBookingTable();
        //searches bookings by date
        //and collects all bookings on dired ddate in an array
        ...
        //fills the slots to display which are already booked
        FillBookingTable();
        //displays the room info to the user
        RoomInfo();
        //creates booking
